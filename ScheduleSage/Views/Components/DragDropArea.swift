  ...
  var body: some View {
    ZStack {
      ...
      RoundedRectangle(cornerRadius: 16)
        .strokeBorder(
          isDragging ? DesignSystem.Colors.primary.opacity(0.6) : Color(.separatorColor).opacity(0),
          lineWidth: isDragging ? 2 : 0.5
        )
        .withVibrancy(
          materialType: .thin,
          cornerRadius: 16,
          addBorder: isDragging,  // 只在拖拽状态显示边框
          opacity: isDragging ? 0.95 : 0.75  // 更高的透明度
        )
        .overlay(
          // 拖放提示效果 - 保留径向渐变
          ZStack {
                ...
                .fill(
                  RadialGradient(
                    gradient: Gradient(colors: [
                      DesignSystem.Colors.primary.opacity(0.15),  // 稍微提高亮度
                      .clear,
                    ]),
                    center: .center,
                    startRadius: 0,
