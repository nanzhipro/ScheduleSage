...
private struct PermissionButton: View {
  ...
  var body: some View {
    Group {
      if !isPermissionGranted {
        VStack(spacing: DesignSystem.Spacing.textSpacing) {
          Text(permissionPurpose)
            .font(DesignSystem.Typography.bodyMedium)
            .foregroundColor(DesignSystem.Colors.secondaryText)
            .multilineTextAlignment(.center)
            .padding(.bottom, DesignSystem.Spacing.elementSpacing / 2)
  ...
  private var permissionPurpose: LocalizedStringKey {
    switch type {
    case .calendar:
      return "onboarding.permission.calendar.purpose"
    }
  }
