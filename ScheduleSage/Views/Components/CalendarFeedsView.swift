    ...
    VStack(spacing: 12) {
      ...
      if viewModel.events.count > maxEventsToShow {
        HStack {
          ...
          Text(
            String(
              format: NSLocalizedString("more_events_count", comment: "还有 %d 个日程"),
              viewModel.events.count - maxEventsToShow
            )
          )
          .font(DesignSystem.Typography.caption)
          .foregroundColor(
            colorScheme == .dark
              ? DesignSystem.Colors.secondaryText.opacity(0.6) : DesignSystem.Colors.secondaryText.opacity(0.6)
          )
          Spacer()
        }
        .padding(.top, 4)
    ...
    .onTapGesture(count: 2) {
      ...
      Task {
        if !event.calendarItemIdentifier.isEmpty {
          _ = await calendarManager.openCalendarEvent(event.calendarItemIdentifier)
        } else {
          // 如果calendarItemIdentifier为空，尝试使用id
          _ = await calendarManager.openCalendarEvent(event.id)
        }
      }
    }
  }
