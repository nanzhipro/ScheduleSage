...
class IAPService: NSObject, ObservableObject {
  ...
  /// 设置设备信息属性
  /// 在配置SDK后和购买前调用，确保属性能被包含在订阅事件中
  private func setDeviceInfoAttributes() {
    logger.info("[IAP] Setting device info attributes")

    // 获取设备信息
    let deviceInfo = DeviceInfoService.getDeviceInfoDictionary()

    // 设置用户属性
    Purchases.shared.attribution.setAttributes(deviceInfo)

    // 记录日志
    logger.info("[IAP] Device info attributes set - Support ID: \(deviceInfo["support_id"] ?? "unknown")")
  }
  ...
  func waitForInitialization() async {
    ...
    return await withCheckedContinuation { continuation in
      let cancellable =
        initializationCompletedSubject
        .first()
        .sink { _ in
          continuation.resume()
        }
  ...
  private func fetchOfferings() async {
    ...
    do {
      ...
      logger.info(
        "[IAP] Current offering packages: \(offerings.current?.availablePackages.map { $0.identifier } ?? [])"
      )
  ...
  private func updateCustomerInfo(_ info: CustomerInfo) {
    Task { @MainActor in
      logger.info(
        "[IAP] Updating customer info - Original App Version: \(info.originalApplicationVersion ?? "unknown")"
      )
      customerInfo = info
      ...
      if let entitlement = info.entitlements[IAPConfiguration.premiumEntitlementId] {
        ...
        logger.info(
          "[IAP] Premium entitlement - Active: \(isActive), Expires: \(expirationDate?.description ?? "none")"
        )
      ...
      logger.info(
        "[IAP] Customer info updated - Premium: \(self.isPremium), Status: \(self.subscriptionStatus.description)"
      )
