...
public final class CalendarManager {
    ...
    private let queue = DispatchQueue(label: "com.schedulesage.calendarmanager",
                                    qos: .userInitiated)
    ...
    public func fetchEvents(from startDate: Date,
                          to endDate: Date,
                          in calendar: EKCalendar? = nil) -> [EKEvent] {
        queue.sync {
    ...
    public func createEvent(title: String,
                          startDate: Date,
                          endDate: Date,
                          in calendar: EKCalendar) -> Result<EKEvent, Error> {
        queue.sync {
    ...
    public func addReminder(to event: EKEvent,
                          alarmOffset: TimeInterval) -> Result<EKEvent, Error> {
        queue.sync {
    ...
    public func searchEvents(matching query: String,
                           from startDate: Date,
                           to endDate: Date) -> [EKEvent] {
        queue.sync {
            let predicate = eventStore.predicateForEvents(
                withStart: startDate,
                end: endDate,
